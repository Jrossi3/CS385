/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Sevryn Napora and Jason Rossi
 * Version     : 1.0
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        while (x != nullptr) {
        	...
        	if (key == x->key()) {
        		...
        		exception_message << "Attempt to insert duplicate key '" << key << "'.";
        		throw tree_exception(exception_message.str());
        		return;
        	}
        	else if (key < x->key()) {
        		x = x->left;
